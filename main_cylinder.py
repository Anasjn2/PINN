# -*- coding: utf-8 -*-
"""
Created on Tue Mar  7 03:35:15 2023
...
circle = dde.geometry.Disk([0.4, 0.35], 0.05)
geom = dde.geometry.CSGDifference(farfield, circle)

inner_rec  = dde.geometry.Rectangle([0.2, 0.2], [0.6, 0.6])
outer_dom  = dde.geometry.CSGDifference(farfield, inner_rec)
outer_dom  = dde.geometry.CSGDifference(outer_dom, circle)
inner_dom  = dde.geometry.CSGDifference(inner_rec, circle)
...
points = np.append(points, circle.random_points(125), axis=0)
...
def boundary_disk(x, on_boundary):
    return on_boundary and (not farfield.on_boundary(x)) and circle.on_boundary(x)
...
bc_disk_u = dde.OperatorBC(geom, fun_no_slip_u, boundary_disk)
bc_disk_v = dde.OperatorBC(geom, fun_no_slip_v, boundary_disk)

bcs = [bc_inlet_u, bc_inlet_v, bc_top_bottom_u, bc_top_bottom_v, bc_outlet_p, bc_disk_u, bc_disk_v]
#%%
...
airfoil_plot = create_airfoil(0, 0, 12, 0.2, 150, offset_x = 0.20, offset_y = 0.35)
...
plt.fill(airfoil_plot[:, 0], airfoil_plot[:, 1])
fig2.colorbar(cnt2)
plt.savefig('cyl_velocity_x.png')
...
plt.fill(airfoil_plot[:, 0], airfoil_plot[:, 1])
fig3.colorbar(cnt3)
plt.savefig('cyl_velocity_y.png')
