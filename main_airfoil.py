...
def create_airfoil(M, P, SS, c, n, offset_x, offset_y):
    """
...
circle = dde.geometry.Disk([0.2, 0.35], 0.15)
geom = dde.geometry.CSGDifference(farfield, circle)

inner_rec  = dde.geometry.Rectangle([0.05, 0.15], [0.35, 0.25])
...
fig1.colorbar(cnt1)
plt.savefig('NACA0012NS_pressure.png')
...
plt.fill(airfoil_plot[:, 0], airfoil_plot[:, 1])
fig2.colorbar(cnt2)
plt.savefig('NACA0012NS_velocity_x.png')
...
plt.fill(airfoil_plot[:, 0], airfoil_plot[:, 1])
fig3.colorbar(cnt3)
plt.savefig('NACA0012NS_velocity_y.png')
